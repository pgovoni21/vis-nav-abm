...
def plot_mult_EA_trends_groups_endonly(groups, val=None, scoring='dist', num_agents=1, max=None, bees=False, trunc=False, title=None, save_name=None, archive=False):
    ...
    if save_name: 
        plt.savefig(fr'{save_data_dir}/{save_name}.png', dpi=100)
    plt.show()
...
def plot_mult_EA_trends_groups_endonly_means(groups, metric_type=None, metric_thresh=None, cmap='berlin', save_name=None):
    ...
    if cmap == 'by_run':
        ...
        # fig, ax1 = plt.subplots(figsize=(3,4)) # solo
    else:
    ...
    for g_num, (group_name, run_names) in enumerate(groups):
        ...
        if cmap == 'by_run':
            ...
            norm = mpl.colors.Normalize(vmin=0, vmax=len(run_names)-1)
            colors = [plt.get_cmap('viridis')(norm(i)) for i in range(len(run_names))]

            for r_num, name in enumerate(run_names):
                dist = np.array(name_to_metric(name, metric_type))
                x = beeswarm(dist)
                im = ax1.scatter(g_num*100 + r_num*2 + x, dist, alpha=.01, c=[colors[r_num]], s=5)
                ax1.scatter(g_num*100 + r_num*2, np.mean(dist), color='k', s=10)
                ax1.scatter(g_num*100 + r_num*2, np.std(dist), color='r', s=10)

                # if np.std(dist) > 150:
                #     print(f'{name} | std: {np.std(dist)}')
            continue
    ...
    if cmap == 'by_run':
        ...
        cbar = ax1.figure.colorbar(im, label='Performance Difference (NS - OG)', extend='both')
    elif cmap == 'dist_shift_ETER':
        cbar = ax1.figure.colorbar(im, label='Performance Difference (BEr - BEt)', extend='both')
    elif cmap == 'dist_JSspatial_NSET':
        cbar = ax1.figure.colorbar(im, label='Directional Divergence', color='red')
    ...
    if 'dist_shift' in metric_type:
        if 'NSETs' in metric_type:
            ax1.set_ylabel('Performance Difference (NS - BEtx5)')
        elif 'NSET' in metric_type:
            ax1.set_ylabel('Performance Difference')
            ax1.yaxis.label.set_color('forestgreen')
        elif 'OGNS' in metric_type:
            ax1.set_ylabel('Performance Difference (NS - OG)')
        if 'ETERs' in metric_type:
            ax1.set_ylabel('Performance Difference (BEr - BEt)x5')
        ...
        if 'NSET' in metric_type:
            ax1.set_ylabel('Directional Divergence')
            ax1.yaxis.label.set_color('red')
        elif 'OGNS' in metric_type:
            ax1.set_ylabel('Directional Divergence (NS - OG)')
        elif 'ETER' in metric_type:
...
def plot_mult_EA_trends_groups_2D(groups, metric_type1=None, metric_type2=None, color_type=None, cbar=True, save_name=None):
    ...
    # index = 10 # _extra, JS_mean_NSET_patch_only
    # index = 8 # _mults, JS_mean_NSETs
    # index = 9 # _mults, JS_mean_ETERs
    ...
    for g_num, (group_name, run_names) in enumerate(groups):
        ...
        for r_num, name in enumerate(run_names):
            ...
            if name in data_dict.keys():
                ...
                # dist = int(name_to_metric(name, 'dist_distance'))

                # if name == 'sc_N3_NRW0_ND2_CNN14_FNN16_vis8_rep12':
                #     print(dist1, dist2)
                ...
                #     # 'sc_N3_NRW0_ND2_CNN14_FNN16_vis8_rep18', # perf + strong spatial + some discernment
                #     'sc_N5_NRW2_ND2_CNN14_FNN16_vis8_rep31', # perf + strong spatial + all discernment
                #     'sc_N6_NRW1_ND4_CNN14_FNN16_vis8_nocoll_rep30', # perf + low spatial
                #     'sc_N6_NRW0_ND5_CNN14_FNN16_vis8_nocoll_rep33', # perf + med spatial
                #     # 'sc_N6_NRW3_ND2_CNN14_FNN16_vis8_nocoll_rep25', # perf + strong spatial
                #     'sc_N5_NRW0_ND4_CNN14_FNN16_vis8_nocoll_rep0', # perf + strong spatial
                #     'sc_N5_NRW2_ND2_CNN14_FNN16_vis8_SinitAg100_rep9', # perf + low spatial
                #     'sc_N6_NRW1_ND4_CNN14_FNN16_vis8_SinitAg100_rep4', # perf + med spatial
                #     'sc_N6_NRW0_ND5_CNN14_FNN16_vis8_SinitAg100_rep35', # perf + strong spatial
                #     'sc_N5_NRW1_ND3_CNN14_FNN16_vis8_collinput_rep34', # perf + low spatial
                #     'sc_N5_NRW0_ND4_CNN14_FNN16_vis8_collinput_rep16', # perf + med spatial
                #     'sc_N5_NRW0_ND4_CNN14_FNN16_vis8_collinput_rep31', # perf + strong spatial
                # ]
                # if name in names:
                ...
                # ETERs = name_to_metric(name, 'shift_ETERs')
                # if ETERs < -200:
                #     print(name, int(dist1), dist2.round(3), int(name_to_metric(name, 'dist_og')), int(ETERs))
                ...
                # if dist1 > 600 and dist2 > 0.33: # NSET - follow - near-direct
        ...
        if len(dist1_group) == 0:
            ...
            # fitogs = np.array(fitog_group)

            label = ''
            if color_type == '':
                im = ax1.scatter(dists1, dists2, c='k', alpha=.5, s=15)
            elif 'heatmap' in color_type:
                ...
                # # min-max/invert fitogs
                # fitog_min = np.min([x for x in fitogs if x is not None])
                # fitog_max = np.max([x for x in fitogs if x is not None])
                # weights = [(fitog - fitog_min)/(fitog_max - fitog_min) for fitog in fitogs if fitog is not None]
                # weights = [1 - w for w in weights]
                # # weighted average 
                # x = np.average([x for x in dists1 if x is not None], weights=weights)
                # y = np.average([x for x in dists2 if x is not None], weights=weights)

                if 'ag' in save_name:
                    if group_name[0] == 'A':
                        im = ax1.scatter(x,y, c=4, cmap='viridis', vmin=0, vmax=4, alpha=.7, s=20)
                    else:
                        im = ax1.scatter(x,y, c=int(group_name[0]), cmap='viridis', vmin=0, vmax=4, alpha=.7, s=20)
                    ...
                    im = ax1.scatter(x,y, c=int(group_name[-1]), cmap='viridis', vmin=0, vmax=2, alpha=.7, s=20)
                    ax1.annotate(group_name[:-1],(x,y), alpha=.3)
                    label = ''
                elif 'Nd0' in save_name:
                    im = ax1.scatter(x,y, c=int(group_name), cmap='viridis', vmin=0, vmax=20, alpha=.7, s=20) #rand
                    ax1.annotate(group_name,(x,y), alpha=.3)
                    label = '# Random'
                else:
                    im = ax1.scatter(x,y, c=int(group_name[0]), cmap='viridis', vmin=0, vmax=5, alpha=.7, s=20) #dir
                    ax1.annotate(group_name,(x,y), alpha=.3)
                    label = '# Direct'

            elif color_type == 'num_X_COM':
                x = np.median([x for x in dists1 if x is not None])
                y = np.median([x for x in dists2 if x is not None])

                total_num = int(group_name[0]) + int(group_name[2:]) # full fig
                if total_num == 5: 
                    xs.append(x)
                    ys.append(y)
                # xs.append(x) # perturb figs
                # ys.append(y)

                if 'ag' in save_name:
                    if group_name[0] == 'A':
                        im0 = ax1.scatter(dists1, dists2, c=[4]*len(run_names), cmap='viridis', vmin=0, vmax=4, alpha=.3, s=10)
                        im = ax1.scatter(x,y, c=4, cmap='viridis', vmin=0, vmax=4, alpha=.7, s=20, edgecolor='black')
                    else:
                        im0 = ax1.scatter(dists1, dists2, c=[int(group_name[0])]*len(run_names), cmap='viridis', vmin=0, vmax=4, alpha=.3, s=10)
                        im = ax1.scatter(x,y, c=int(group_name[0]), cmap='viridis', vmin=0, vmax=4, alpha=.7, s=20, edgecolor='black')
                    label = 'Init Diameter from Agent'
                elif 'res' in save_name:
                    if group_name[0] == 'A':
                        im0 = ax1.scatter(dists1, dists2, c=[4]*len(run_names), cmap='viridis', vmin=0, vmax=4, alpha=.3, s=10)
                    ...
                    im0 = ax1.scatter(dists1, dists2, c=[int(group_name)]*len(run_names), cmap='viridis', vmin=1, vmax=20, alpha=.3, s=10)
                    im = ax1.scatter(x,y, c=int(group_name), cmap='viridis', vmin=1, vmax=20, alpha=.7, s=20, edgecolor='black')
                    label = '# Random'
                else:
                    im0 = ax1.scatter(dists1, dists2, c=[int(group_name[0])]*len(run_names), cmap='viridis', vmin=0, vmax=5, alpha=.3, s=10)
                    if total_num == 5: im = ax1.scatter(x,y, c=int(group_name[0]), cmap='viridis', vmin=0, vmax=5, alpha=.7, s=20, edgecolor='black')
                    label = '# Direct'
                ...
                im = ax1.scatter(dists1, dists2, c=[int(group_name[2:])]*len(run_names), cmap='viridis', vmin=0, vmax=5, alpha=.3, s=10)
                # im = ax1.scatter(dists1, dists2, c=[int(group_name[2:])]*len(run_names), cmap='viridis', vmin=0, vmax=20, alpha=.3, s=10)
                label = '# Random'
                ...
                # im = ax1.scatter(dists1, dists2, c=data, cmap='viridis', vmin=225, vmax=275, alpha=.7, s=20) # for indiv plots + nocoll
                ...
                if 'Ag' in group_name:
                    im = ax1.scatter(dists1, dists2, c=[int(group_name[6])]*len(run_names), cmap='viridis', vmin=1, vmax=4, alpha=.3, s=10)
                ...
                # data = data > 1
                # # im = ax1.scatter(dists1, dists2, c=data, cmap='viridis', vmin=0, vmax=100, alpha=.3, s=10)
                # im = ax1.scatter(dists1, dists2, c=data, cmap='viridis', norm=mpl.colors.LogNorm(vmin=1, vmax=400), alpha=.3, s=10)
                # label = 'Remaining Distance to Patch'
                # print(f'{group_name}: {np.sum(data<1)} out of {len(data)}')
    ...
    if 'NSET' in metric_type1:
        ...
        ax1.set_xlabel('Performance Difference (NS - OG)')
        ax1.set_ylabel('Directional Divergence (NS - OG)')

    if 'heatmap' in color_type:
        ...
        if 'taskdep' in color_type:
            H[:11,:] = 0 # skip perf_diff < 120
        ...
        # scatter plots
        ax1.set_xlim(-500,850) # full fig
        # ax1.set_xlim(-200,850) # perturb figs
        ax1.set_ylim(-.01,.43)
        if cbar:
            if label == '':
    ...
    plt.tight_layout()
    if save_name: 
        plt.savefig(fr'{data_dir}/{save_name}_2D_{metric_type1}_x_{metric_type2}_{color_type}.png', dpi=100)
    plt.close()
...
def relative_stacked_bars_social(groups, metric_type1, metric_type2, save_name, dpi=100):
    ...
    for g_num, (group_name, run_names) in enumerate(groups):
        ...
        runtype_by_category[group_name] = [navs, hybs, folls]
        category_by_runtype['Navigator'].append(navs)
        category_by_runtype['Hybrid'].append(hybs)
        category_by_runtype['Follower'].append(folls)
    
    # print(category_by_runtype)
    # print(runtype_by_category)

    # for cat,sums in runtype_by_category.items():
    #     print(np.array(sums).sum())
    ...
    for i, (category, category_count) in enumerate(category_by_runtype.items()):
        ...
        if len(colors) == 1:
            ...
            bottom += category_count_normalized
        if len(colors) == 2:
            ax.bar(run_types, category_count_normalized, label=category, bottom=bottom, facecolor=colors[0], edgecolor=colors[1], hatch=r'\\', hatch_linewidth=5)
            bottom += category_count_normalized

    ax.set_ylabel('Relative Occurence')
    ax.set_xlabel(f'# Direct x # Random')
    ax.set_ylim(0,1.05)
    ax.legend(loc='lower left', reverse=True)
    # ax.legend(loc=(.85,.7))

    plt.tight_layout()
    if 'OGNS' in metric_type1:
        plt.savefig(fr'{data_dir}/relative_occurence_social_bars_OGNS_{save_name}_{dpi}.png', dpi=dpi)
    else:
        plt.savefig(fr'{data_dir}/relative_occurence_social_bars_NSET_{save_name}_{dpi}.png', dpi=dpi)
    plt.show()
    # plt.close()



def plot_social_pies(groups, metric_type1, metric_type2, plot_type, save_name, dpi=100):
        ...
        for r_num, name in enumerate(run_names):
            if name in data_dict.keys():
                ...
                if plot_type == 'fits':
                    ...
                    extra = name_to_metric(name, 'shift_ETER')
                elif plot_type == 'ETERs':
                    extra = name_to_metric(name, 'shift_ETERs')
                elif plot_type == 'dirent_OG':
                    extra = name_to_metric(name, 'dirent_OG')
                elif plot_type == 'dirent_ET':
                    extra = name_to_metric(name, 'dirent_ET')
                elif plot_type == 'dirent_NS':
                    extra = name_to_metric(name, 'dirent_NS')
                elif plot_type == 'learning_time':
                    extra = name_to_metric(name, 'learning_time')
                elif plot_type == 'Nd+2':
                ...
                # elif dist1 > 512.5 and dist2 > .155:
                    ...
                    hybs += 1
                    # hybs_extra += extra
                    hybs_extra.append(extra)
            else:
                print(f'{name} not in dict')
        # runtype_by_category[group_name] = [navs, hybs, folls], [navs_extra, hybs_extra, folls_extra]
        navs_extra = np.array(navs_extra)
    ...
    red = plt.cm.Reds(np.linspace(0, 1, 256))[-50]
    fade = colors.LinearSegmentedColormap.from_list('', [vir[0], red], N=256)
    fade_arr = fade(np.linspace(.75,0,256))
    virfade = colors.LinearSegmentedColormap.from_list('', np.vstack((fade_arr, vir)))

    if plot_type == 'types':
        colors = ['cornflowerblue', 'mediumorchid', 'orange']
    elif plot_type == 'fits':
        norm = plt.Normalize(vmin=225, vmax=450)
    ...
    for i in range(6):
        for j in range(6):
            ...
            # string = f'sc_N{i+j+1}_NRW{i}_ND{j}_CNN14_FNN16_vis8'
            string = f'{j}x{i}'
            if string in runtype_by_category:                
                counts, extra = runtype_by_category[string]
                counts = np.array(counts)
                extra = np.array(extra)
                # extra = extra/counts # normalize

                # print(string, counts, extra)
                # ax.set_title(f'({i},{j})', fontsize=10)

                if plot_type == 'types':
                    wedges,_ = ax.pie(counts, colors=colors, startangle=90)
                else:
                    wedges,_ = ax.pie(counts, colors=colormap(norm(extra)), startangle=90)

            else:
                ax.axis('off')
    
    # all_fits = np.array(all_fits)
    # print(np.nanmin(all_fits), np.nanmax(all_fits))

    # axes.set_xlabel('# Direct')
...
### ----------social triangle----------- ###
    ...
    # plot_social_table_DR(metric_type='fit_og', extra='nocoll', dpi=100)
    # plot_social_table_DR(metric_type='fit_nosoc', extra='nocoll', dpi=100)
    # plot_social_table_DR(metric_type='fit_exploiter', extra='nocoll', dpi=100)
    # plot_social_table_DR(metric_type='meanshift_OGNS', extra='nocoll', dpi=100)
    ...
    # # plot_social_table_DR(metric_type='meanshift_Nr+2', extra='nocoll', dpi=100)

    # plot_social_table_DR(metric_type='fit_og', extra='SinitAg100', dpi=100)
    # plot_social_table_DR(metric_type='fit_nosoc', extra='SinitAg100', dpi=100)
    # plot_social_table_DR(metric_type='fit_exploiter', extra='SinitAg100', dpi=100)
    # plot_social_table_DR(metric_type='meanshift_OGNS', extra='SinitAg100', dpi=100)
    ...
    # plot_social_table_DR(metric_type='fit_og', extra='collinput', dpi=100)
    # plot_social_table_DR(metric_type='fit_nosoc', extra='collinput', dpi=100)
    # plot_social_table_DR(metric_type='fit_exploiter', extra='collinput', dpi=100)
    # plot_social_table_DR(metric_type='meanshift_OGNS', extra='collinput', dpi=100)
...
### ----------social beeswarms----------- ###
    ...
    # save_name='groups_sc_res_CNN14_FNN16_vis8'
    ...
    if 'Nall_C' in save_name:
        ...
        groups.append(('All-Map', [f'sc_N2_NRW0_ND1_CNN14_FNN16_vis8_rep{x}' for x in range(n)]))
    ...
    elif 'Nall_collinput' in save_name:
        groups.append(('0x0', [f'sc_N1_NRW0_ND0_CNN14_FNN16_vis8_rep{x}' for x in range(n)]))
        ...
        groups.append(('4x1',[f'sc_N6_NRW1_ND4_CNN14_FNN16_vis8_collinput_rep{x}' for x in range(n)]))
        groups.append(('5x0',[f'sc_N6_NRW0_ND5_CNN14_FNN16_vis8_collinput_rep{x}' for x in range(n)]))
    elif 'Nall_SinitAg' in save_name:
        groups.append(('0x0', [f'sc_N1_NRW0_ND0_CNN14_FNN16_vis8_rep{x}' for x in range(n)]))
        ...
        groups.append(('4x1',[f'sc_N6_NRW1_ND4_CNN14_FNN16_vis8_SinitAg100_rep{x}' for x in range(n)]))
        groups.append(('5x0',[f'sc_N6_NRW0_ND5_CNN14_FNN16_vis8_SinitAg100_rep{x}' for x in range(n)]))
    elif 'Nd5_varycoll' in save_name:
    ...
    # # test = 'dist_shift_OGNS'
    # test = 'dist_shift_NSET'
    # # test = 'dist_shift_NSER'
    # # test = 'dist_shift_ETER'
...
### ----------pop runs----------- ###
    ...
    # names = [f'sc_N21_NRW20_ND0_CNN14_FNN16_vis8_nocoll_rep{x}' for x in range(40)]
    # plot_mult_EA_trends(names, val='cen', save_name='0x20')
...
### ----------group pop runs----------- ###
    ...
    # plot_mult_EA_trends_groups_endonly(groups, val='cen', archive=True, save_name='groups_endonly_singlecorner_boundary_scale')
