...
class Simulation:
    ...
    def __init__(self, env_size, window_pad,
        ...
        self.boundary_endpts = [
            np.array([ self.x_min, self.y_min ]), # TL
            np.array([ self.x_max, self.y_min ]), # TR
            np.array([ self.x_min, self.y_max ]), # BL
            np.array([ self.x_max, self.y_max ]) # BR
        ]
...
### -------------------------- DRAWING FUNCTIONS -------------------------- ###
    ...
    def draw_landmarks(self):
        for landmark in self.landmarks:
            pygame.draw.circle(self.screen, landmark.color, landmark.position + self.window_pad, landmark.radius)
        ...
        for agent in self.agents:
            ...
            if self.vis_transform:
                ...
                for phi, vis_name, dist, dist_input in zip(agent.phis, agent.vis_field, agent.dist_field, agent.dist_input):
                    ...
                    elif vis_name == 'BL': # --> green
                        pygame.draw.circle(
                            self.screen, colors.LIME, 
                            (start_pos[0] + np.cos(agent.orientation - phi) * vis_proj_distance,
                            start_pos[1] - np.sin(agent.orientation - phi) * vis_proj_distance),
                            radius = (dist_input+1)*vis_project_IDbubble_size)
                    elif vis_name == 'TR': # --> blue
                        pygame.draw.circle(
                            self.screen, colors.CORN, 
                            (start_pos[0] + np.cos(agent.orientation - phi) * vis_proj_distance,
                            start_pos[1] - np.sin(agent.orientation - phi) * vis_proj_distance),
                            radius = (dist_input+1)*vis_project_IDbubble_size)
                    elif vis_name == 'BR': # --> yellow
                        pygame.draw.circle(
    ...
    def draw_frame(self):
        ...
        self.landmarks.draw(self.screen)
...
### -------------------------- LANDMARK FUNCTIONS -------------------------- ###
    ...
    def create_landmarks(self):
        ...
        landmark_info = [
            ('TL', colors.TOMATO, np.array([ self.x_min, self.y_min ])),
            ('BL', colors.LIME, np.array([ self.x_min, self.y_max ])),
            ('TR', colors.CORN, np.array([ self.x_max, self.y_min ])),
            ('BR', colors.GOLD, np.array([ self.x_max, self.y_max ]))
        ]

        for id, color, position in landmark_info:
            landmark = Landmark(
                id=id,
                color=color,
...
### -------------------------- AGENT FUNCTIONS -------------------------- ###
    ...
    def create_agents(self):
        ...
        """
        # screen edge spawning buffer by agent diameter
        x_min, x_max = self.x_min + 2*self.agent_radii, self.x_max - 2*self.agent_radii
        y_min, y_max = self.y_min + 2*self.agent_radii, self.y_max - 2*self.agent_radii

        if self.N == 1:

            colliding_landmarks = [0]
            colliding_resources = [0]

            retries = 0 
            while len(colliding_landmarks) > 0 or len(colliding_resources) > 0:
            ...
            for i in range(self.N):
                ...
                retries = 0
                while len(colliding_landmarks) > 0 or len(colliding_resources) > 0 or len(colliding_agents) > 0:
...
### -------------------------- COLLISION FUNCTIONS -------------------------- ###
    ...
    def collide_agent_res(self):
        ...
        collision_group = pygame.sprite.groupcollide(self.agents, self.resources, False, False, pygame.sprite.collide_circle)

        # Switch on all agents currently on a resource 
        for agent, resource_list in collision_group.items():
    ...
    #     collision_group = pygame.sprite.groupcollide(self.agents, self.walls, False, False)

    #     # Change agent mode + note points of contact (carry out velocity-stopping check later in agent.move())
    #     for agent, wall_list in collision_group.items():
    ...
    def collide_agent_landmark(self):

        # Create dict of every agent that has collided : [colliding landmarks]
        collision_group = pygame.sprite.groupcollide(self.agents, self.landmarks, False, False, pygame.sprite.collide_circle)

        # Carry out agent-landmark collisions + generate list of collided landmarks
        for agent, landmark_list in collision_group.items():
            ...
            for landmark in landmark_list:
                ...
                mid_pos = (agent.position + landmark.position) / 2
    ...
    def collide_agent_agent(self):
        ...
        collision_group = pygame.sprite.groupcollide(self.agents, self.agents, False, False, supcalc.within_group_collision)
        
        # Carry out agent-agent collisions + generate list of collided agents
        for agent1, other_agents in collision_group.items():
    ...
    def start(self):
        ...
        while self.t < self.T:
            ...
            if not self.is_paused:
                ...
                self.collide_agent_landmark()
                self.collide_agent_agent()
                self.collide_agent_res()

                # Update visual projections
                for agent in self.agents:
                    agent.visual_sensing(self.landmarks, self.agents)
