...
class Simulation:
    ...
    def __init__(self, env_size, window_pad,
                 N, N_rand, T, with_visualization, framerate, print_enabled, plot_trajectory, log_zarr_file, save_ext,
                 ...
                 boundary_scale, sim_type, RW_rot_diff
                 ):
        """
                ...
                for agent in self.agents:
                    ...
                    if agent.id == 0:
                        ...
                        if agent.mode == 'exploit':
                            ...
                            # if agent.id < self.N_rand: # (via RW) # old way +1 direct
                            if agent.id < self.N_rand + 1: # (via RW) # new way
                                agent.move((2*self.rot_diff)**.5 * np.random.uniform(-1,1))
                                ...
                                if agent.collide_timer > 0:
                                    ...
                                    angle_to_food = np.arctan2(-(self.resources.sprites()[0].position[1] - agent.position[1]),
                                                            self.resources.sprites()[0].position[0] - agent.position[0])
