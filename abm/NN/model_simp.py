...
from abm.NN.vision_simp import ConvNeXt as CNN
from abm.NN.vision_simp import LayerNorm
...
class WorldModel(nn.Module):
    ...
    def assign_params(self, param_vector):
        ...
        for m in self.modules():
            if isinstance(m, (nn.Conv1d, nn.LayerNorm, LayerNorm, nn.Linear, nn.GRU)):
    ...
    def _init_weights(self):
        for m in self.modules():
            if isinstance(m, (nn.Conv1d, nn.LayerNorm, LayerNorm, nn.Linear, nn.InstanceNorm1d, nn.GRU)):
                for p in m.parameters():
...
if __name__ == '__main__':
    ...
    for m in model.modules():
        if isinstance(m, (nn.Linear, nn.InstanceNorm1d, nn.GRU, nn.LSTM, nn.Conv1d, nn.LayerNorm, LayerNorm)):
